...
{
    public partial class Principal_Circulacion_Encargado_borrador : Form
    {
        ...
        public Principal_Circulacion_Encargado_borrador()
        {
            InitializeComponent();
        ...
        {
            Principal_Encargado form_Principal = new Principal_Encargado();
            form_Principal.Show();
            this.Hide();
        }
        ...
        private void guna2Button2_Click(object sender, EventArgs e)
        {
            ...
            string resultado = solicitudService.RegistrarSolicitud(
                identificacion,
                codigoInterno,
                1); // Ahora pasamos todos los parámetros necesarios
            CargarDatosDesdeBaseDeDatos();
            // Mostrar el resultado al usuario
            MessageBox.Show(resultado, "Resultado", MessageBoxButtons.OK, MessageBoxIcon.Information);


            // Preguntar si desea agregar otro producto para la misma identificación
            ...
            {
                // Si elige "Sí", solo actualizar el ComboBox de productos
                LlenarComboBoxProductos(cbCodigoInterno, "Disponible");
                LlenarComboBoxProductos(cbCodigoInternoDevolucion, "No disponible");
                cbCodigoInterno.SelectedIndex = -1; // Limpiar la selección actual para permitir al usuario elegir otro producto
            ...
            {
                // Si elige "No", actualizar ambos ComboBox: productos e identificación
                LlenarComboBoxProductos(cbCodigoInterno, "Disponible");
                LlenarComboBoxProductos(cbCodigoInternoDevolucion, "No disponible");
                LlenarComboBoxTerceros(cbIdentificacion);   // Actualizar identificación
                if (cbIdentificacion.Items.Count > 0)
                {
                    cbIdentificacion.SelectedIndex = 0; // Seleccionar el primer tercero por defecto (si lo hay)
                }
                if (cbCodigoInterno.Items.Count > 0)
                {
                    cbCodigoInterno.SelectedIndex = 0; // Seleccionar el primer tercero por defecto (si lo hay)
        ...
        private void cbCodigoInterno_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cbCodigoInterno.SelectedItem is KeyValuePair<string, string> selectedItem)
            {
                ...
                // Realiza las acciones correspondientes con la selección
            }
        }
        ...
        private void btnDevolver_Click(object sender, EventArgs e)
        {
            ...
            var selectedItemCodigoCliente = cbCodigoInternoDevolucion.SelectedItem as dynamic;
            string codigoInterno = selectedItemCodigoCliente?.Value;


            var selectedItemIdentificacio = cbSolicitanteDevolver.SelectedItem as dynamic;
            string identificacion = selectedItemIdentificacio?.Value;
            ...
            LlenarComboBoxTerceros(cbSolicitanteDevolver);   // Actualizar identificación

            if (cbCodigoInternoDevolucion.Items.Count > 0)
            {
                cbCodigoInternoDevolucion.SelectedIndex = 0; // Seleccionar el primer tercero por defecto (si lo hay)
            }


            cbCodigoInternoDevolucion.SelectedIndex = -1;
        }
        ...
        private void cbSolicitanteDevolver_SelectedIndexChanged(object sender, EventArgs e)
        {
